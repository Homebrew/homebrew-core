class Libglib < Formula
  ...
  def install
    ...
    # ref: https://github.com/conda-forge/glib-feedstock/blob/main/recipe/install.sh
    (
      Dir.glob(bin/"{gapplication,gdbus*,gio*,glib-*,gobject*,gresource,gsettings,gtester*}") +
      Dir.glob(share/"aclocal/{glib-*,gsettings*}") +
      Dir.glob(share/"bash-completion/completions/{gapplication,gdbus,gio,gresource,gsettings}") +
      Dir.glob(share/"gettext/its") +
      Dir.glob(share/"glib-*") +
      Dir.glob(share/"locale/*")
    ).each do |file|
      rm_rf file
    end
  end
