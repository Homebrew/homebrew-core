class ZabbixCli < Formula
  ...
  def install
    # `shellingham` auto-detection doesn't work in CI environment so defer
    # installation in order to force `typer` to use argument as shell
    ...
    # TESTING - Remove
    system "cat", "#{bash_completion}/zabbix-cli"
    system "cat", "#{zsh_completion}/_zabbix-cli"
    system "cat", "#{fish_completion}/zabbix-cli.fish"
    odie "TESTING"
  end
