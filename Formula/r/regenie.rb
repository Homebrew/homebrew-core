class Regenie < Formula
  ...
  # formula. Version pinned by upstream's Dockerfile. The upstream Fossil server
  # (code.enkre.net) often answers HTTP 508, so use the Git mirror of its
  # repository (same tree as the v1.1.7 Fossil tarball, also used by conda-forge).
  resource "bgen" do
    url "https://github.com/nebfield/bgen-git-mirror/archive/refs/tags/v1.1.7.tar.gz"
  ...
  def install
    ...
    # which breaks the final link on arm64. Fixed upstream, remove after 4.1.3:
