class Envoy < Formula
  ...
  def install
    ...
    # Override the default clang to use the one from llvm@18 in PATH
    env_path = "#{llvm.opt_bin}:" + env_path
    ...
    args = %W[
      ...
      --action_env=CC=#{llvm.opt_bin}/clang
      --action_env=CXX=#{llvm.opt_bin}/clang++
      --action_env=BAZEL_USE_CPP_ONLY_TOOLCHAIN=1
    ]

    if OS.mac?
      # Make sure to use c90 for zlib
      args << "--per_file_copt='external/.*zlib.*/.*.c@-std=c90'"
      args << "--host_per_file_copt='external/.*zlib.*/.*.c@-std=c90'"
      # Use llvm's static libs
      args << "--linkopt=#{llvm.opt_lib}/libunwind.a"
      args << "--linkopt=#{llvm.opt_lib}/c++/libc++.a"
      args << "--linkopt=#{llvm.opt_lib}/c++/libc++abi.a"
      args << "--dynamic_mode=off"
      args << "--action_env=HOMEBREW_LIBRARY_PATHS=#{llvm.opt_bin}:#{llvm.opt_bin}/c++"
    end
