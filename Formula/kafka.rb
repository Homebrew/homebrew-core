class Kafka < Formula
  ...
  test do
    ...
    begin
      ...
      system "#{bin}/kafka-topics --zookeeper localhost:2181 --create --if-not-exists --replication-factor 1 --partitions 1 --topic test > #{testpath}/kafka/demo.out"
      pipe_output("#{bin}/kafka-console-producer --broker-list localhost:9092 --topic test", "test message")
      system "#{bin}/kafka-console-consumer --zookeeper localhost:2181 --topic test --from-beginning --max-messages 1 >> #{testpath}/kafka/demo.out"
