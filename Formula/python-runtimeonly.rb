class PythonRuntimeonly < Formula
  # Sync this formula with the latest version of Python
  # and delete the corresponding segments for the `pip` packages
  ...
  keg_only "only used as runtime-only dependency for other formulae"
  ...
  depends_on "readline"
  depends_on "sqlite" # `ALLOWED_USES_FROM_MACOS_DEPS` need to be updated in Homebrew/brew
  depends_on "xz"
  ...
  def site_packages
    HOMEBREW_PREFIX/"lib/python-runtimeonly/site-packages"
  end
  ...
  def post_install
    ...
    # Create a site-packages in HOMEBREW_PREFIX/lib/python-runtimeonly/site-packages
  ...
  def sitecustomize
    <<~EOS
      ...
      if os.path.realpath(sys.executable).startswith('#{rack}'):
          ...
          sys.path = [long_prefix.sub('#{HOMEBREW_PREFIX/"lib/python-runtimeonly/site-packages"}', p) for p in sys.path]
