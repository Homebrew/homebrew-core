class PythonDebugAT312 < Formula
  ...
  keg_only "it conflicts with python@3.12"
  ...
  # The prefix location of site-packages.
  def site_packages
    prefix/"lib/python#{version.major_minor}/site-packages"
  end
  ...
  def install
    ...
    # Resolve prefix in our sysconfig modification.
    inreplace "Lib/sysconfig.py", "@@HOMEBREW_PREFIX@@", prefix
    ...
    if OS.mac?
      ...
      bad_cellar_path_files = [
        lib_cellar/"_sysconfigdata_d_darwin_darwin.py",
        lib_cellar/"config-#{version.major_minor}d-darwin/Makefile",
      ...
      inreplace lib_cellar/"config-#{version.major_minor}d-darwin/Makefile",
      ...
      inreplace lib_cellar/"_sysconfigdata_d_darwin_darwin.py",
      ...
      inreplace Dir[lib_cellar/"**/_sysconfigdata_*linux_x86_64-*.py",
                    ...
                    lib/"pkgconfig/python-#{version.major_minor}{,-embed}.pc"],
                prefix, opt_prefix
    ...
    # Install unversioned and major-versioned symlinks in libexec/bin.
    {
      ...
      "idle3d"                               => "idle#{version.major_minor}",
      "idle#{version.major_minor}d"          => "idle#{version.major_minor}",
      "pydoc"                                => "pydoc#{version.major_minor}",
      "pydoc3"                               => "pydoc#{version.major_minor}",
      "pydoc3d"                              => "pydoc#{version.major_minor}",
      "pydoc#{version.major_minor}d"         => "pydoc#{version.major_minor}",
      "python"                               => "python#{version.major_minor}",
      "python3"                              => "python#{version.major_minor}",
      "python3d"                             => "python#{version.major_minor}",
      "python#{version.major_minor}d"        => "python#{version.major_minor}",
      "python-config"                        => "python#{version.major_minor}-config",
      "python3-config"                       => "python#{version.major_minor}-config",
      "python3d-config"                      => "python#{version.major_minor}-config",
      "python#{version.major_minor}d-config" => "python#{version.major_minor}-config",
  ...
  def post_install
    ...
    # Install unversioned symlinks in libexec/bin.
    {
      ...
      "pip3d"                        => "pip#{version.major_minor}",
      "pip#{version.major_minor}"    => "pip#{version.major_minor}",
      "pip#{version.major_minor}d"   => "pip#{version.major_minor}",
      "wheel"                        => "wheel#{version.major_minor}",
      "wheel3"                       => "wheel#{version.major_minor}",
      "wheel3d"                      => "wheel#{version.major_minor}",
      "wheel#{version.major_minor}"  => "wheel#{version.major_minor}",
      "wheel#{version.major_minor}d" => "wheel#{version.major_minor}",
    }.each do |short_name, long_name|
      (libexec/"bin").install_symlink (bin/long_name).realpath => short_name
    end
  end
  ...
  def caveats
    <<~EOS
      Python debug build has been installed as
        #{opt_prefix}/bin/python#{version.major_minor}

      Unversioned and major-versioned symlinks `python`, `python3`, `python-config`, `python3-config`, `pip`, `pip3`, etc. pointing to
      `python#{version.major_minor}d`, `python#{version.major_minor}d-config`, `pip#{version.major_minor}d` etc., respectively, have been installed into
        #{opt_libexec}/bin

      You can install Python packages with a virtual environment:

         #{opt_libexec}/bin/python3d -m venv path/to/venv
  ...
  test do
    # Check if the abiflags contain `d`
    assert_match(/d/,
      shell_output("#{python3} -Sc 'import sys; print(sys.abiflags)' 2>/dev/null"))

    # Check if the list of default warning filters is empty
    assert_match "[]",
                 shell_output("#{python3} -Sc 'import warnings; print(warnings.filters)'")
    assert_match "PendingDeprecationWarning: message",
                 shell_output(
                   <<~EOS,
                     #{python3} -Sc \
                        'import warnings; warnings.warn("message", category=PendingDeprecationWarning)' 2>&1
                   EOS
                 )

    # Check if sys module has member gettotalrefcount
    system python3, "-c", "import sys; sys.gettotalrefcount()"
