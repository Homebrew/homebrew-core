class PythonAT313t < Formula
  ...
  def lib_cellar
    on_macos do
      return frameworks/"Python.framework/Versions/#{version.major_minor}t/lib/python#{version.major_minor}"
    end
    on_linux do
  ...
  def install
    ...
    if OS.mac?
      ...
      pc_dir = frameworks/"Python.framework/Versions/#{version.major_minor}t/lib/pkgconfig"
    ...
    {
      "idle"          => "idle#{version.major_minor}t",
      "pydoc"         => "pydoc#{version.major_minor}t",
      "python"        => "python#{version.major_minor}t",
  ...
  def post_install
    ...
    %W[wheel3 pip3 wheel#{version.major_minor}t pip#{version.major_minor}].each do |e|
  ...
  def sitecustomize
    <<~EOS
      ...
      if os.path.realpath(sys.executable).startswith('#{rack}'):
          ...
          long_prefix = re.compile(r'#{rack}/[0-9\\._abrc]+/Frameworks/Python\\.framework/Versions/#{version.major_minor}t/lib/python#{version.major_minor}/site-packages')
