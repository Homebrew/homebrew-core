class Gdtoolkit < Formula
  ...
  test do
    ...
    assert_equal expected, shell_output("#{bin}/gdformat --diff #{testpath}/gdformat.gd 2>&1", 1)

    # gdlint
    (testpath/"gdlint.gd").write "func notValid(): pass"
    expected = "#{testpath}/gdlint.gd:1: Error: Function name \"notValid\" is not valid (function-name)\nFailure: 1 problem found\n"
    assert_equal expected, shell_output("#{bin}/gdlint #{testpath}/gdlint.gd 2>&1", 1)
