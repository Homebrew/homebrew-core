class OpentimestampsClient < Formula
  ...
  conflicts_with "ots", because: "both install `ots` binaries"

  pypi_packages package_name: "opentimestamps-client", exclude_packages: "certifi"
  ...
  test do
    ...
    verify_output = shell_output(
      "#{bin}/ots --no-cache --no-default-whitelist verify -f input.txt input.txt.ots 2>&1", 1
    )
    assert_match(
      "Ignoring attestation from calendar https://calendar.example: Calendar not in whitelist",
      verify_output,
    )
  end
end
