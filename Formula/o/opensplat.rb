class Opensplat < Formula
  ...
  test do
    ...
    # Test GPU support: should be built or gracefully fallback to CPU
    gpu_output = shell_output(
      "#{bin}/opensplat #{testpath} --num-iters 1 --downscale-factor 1 --num-downscales 0 2>&1 || true",
    )
    if OS.mac?
      refute_empty gpu_output, "Should emit output when probing the default backend"
    else
      # Linux builds may use CUDA/HIP, or fall back to/report CPU-only builds.
      assert_match(/Using CPU|Using CUDA|Using HIP|GPU support not built/i, gpu_output,
                   "Should use an available backend or report missing GPU support")
    end
