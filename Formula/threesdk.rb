class Threesdk < Formula
  ...
  desc "Software development kit for Threefold and JumpscaleX"
  homepage "https://sdk.threefold.io/"
  url "https://github.com/threefoldtech/jumpscaleX_core/archive/v10.5.3.tar.gz"
  sha256 "4c0639f533a5231cda572e4a3f65848ab43b9c2ffa27484a6081828958b89127"
  ...
  def install
    ...
    %w[
      docopt jedi parso prompt-toolkit ptpython Pygments six wcwidth certifi
      chardet idna requests urllib3 altgraph macholib PyInstaller packaging
      pyparsing pudb urwid cffi pycparser PyNaCl
    ].each do |r|
      venv.pip_install resource(r)
    end
    chdir "install" do
      ENV["PATH"] += ":/usr/local/bin" # needed for python
      # inreplace doesn't work
      system 'version=$(git describe --tags); gsed -i "s/_unreleased_/${version}/" threesdk/__init__.py;'
      system "pyinstaller", "3sdk.spec"
      bin.install "dist/3sdk"
    end
  end

  test do
    # make sure binary is running ok
    system bin / "3sdk", "--expert", "info"
    assert_match "builder: manage container images", shell_output("#{bin}/3sdk info").strip
    # install requires docker to be installed by user if not it will fail
    system bin / "3sdk container install explorer=none secret=123456"
    assert_predicate Dir.home "/sandbox", :exist
  end
end
