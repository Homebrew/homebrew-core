  ...
  def install
    resource("mingw-w64").stage do
      ...
      target_archs.each do |arch|
        ...
        (arch_dir/target).install_symlink prefix/"mingw/include"
    ...
    target_archs.each do |arch|
      ...
      # Link 'as' to 'clang'
      # (LLVM doesn't come with a standalone assembler, but clang may be enough for some uses)
      bin.install_symlink llvm.opt_bin/"clang" => "#{target}-as"

      # Create ld wrapper script
      # Ideally lld would figure this out itself when run through a triplet-prefixed symlink
      lld_target = case arch
      when "i686" then "i386pe"
      when "x86_64" then "i386pep"
      when "aarch64" then "arm64pe"
      when "arm64ec" then "arm64ecpe"
      else arch
      end
      (bin/"#{target}-ld").atomic_write <<~SH
        #!/bin/sh
        #{formula_opt_bin("lld")}/ld.lld -m #{lld_target} "$@"
      SH
    end
  end
