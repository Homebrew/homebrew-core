class Llgo < Formula
  ...
  url "https://github.com/goplus/llgo/archive/refs/tags/v0.10.0.tar.gz"
  sha256 "a47af34904645ddf26d69018a7b44ec0350f41b1a860bd1d8f4bddebc27cde80"
  license "Apache-2.0"
  ...
  def lib_deps
    %w[bdw-gc openssl libffi].map { |name| find_dep(name).opt_lib }
  end

  def path_deps
    %w[llvm go pkgconf].map { |name| find_dep(name).opt_bin }
  end
  ...
  test do
    ...
    (testpath/"go.mod").write <<~GOMOD
      module hello
    GOMOD
    system go.opt_bin/"go", "get", "github.com/goplus/llgo@v#{version}"
