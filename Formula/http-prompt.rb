class HttpPrompt < Formula
  ...
  test do
    downloaded_file = (testpath/"http-prompt.tar.gz")

    (testpath/"test.exp").write <<~EOS
      #!/usr/bin/env expect -f
      set timeout -1

      spawn #{bin}/http-prompt #{stable.url}

      send -- "get > #{downloaded_file}\r"
      sleep 1
      send -- "exit\r"
      expect eof
    EOS
    ...
    assert_predicate downloaded_file, :exist?
    downloaded_file.verify_checksum stable.checksum
  end
end
