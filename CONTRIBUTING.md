...
* `brew bump-formula-pr --strict foo` with `--url=...` and `--sha256=...`, or `--tag=...` and `--revision=...` arguments (and optionally `--version=...` as well). Run `brew bump-formula-pr --help` for additional instructions.
